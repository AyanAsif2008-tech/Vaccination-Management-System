...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        if ($name === '') {
            ...
            try {
                if ($action === 'create') {
                    $stmt = $pdo->prepare("INSERT INTO vaccine (Vaccine_Name, Description, Age_Group, Stock_Status, Hospital_ID) VALUES (?,?,?,?,?)");
                    $stmt->execute([$name, $desc, $ageGrp, $stock, $hospitalId]);
                    flash_set('success', 'Vaccine added to your inventory.');
                } else {
                    // Scoped to this hospital's own vaccines only
                    $id = (int)$_POST['vaccine_id'];
                    $stmt = $pdo->prepare("UPDATE vaccine SET Vaccine_Name=?, Description=?, Age_Group=?, Stock_Status=? WHERE Vaccine_ID=? AND Hospital_ID=?");
                    $stmt->execute([$name, $desc, $ageGrp, $stock, $id, $hospitalId]);
    ...
    if ($action === 'delete') {
        $id = (int)$_POST['vaccine_id'];
        try {
            $stmt = $pdo->prepare("DELETE FROM vaccine WHERE Vaccine_ID = ? AND Hospital_ID = ?");
            $stmt->execute([$id, $hospitalId]);
            flash_set('success', 'Vaccine removed from your inventory.');
        } catch (PDOException $ex) {
...
$params = [$hospitalId];
if ($q !== '') {
    $where .= " AND (Vaccine_Name LIKE ? OR Age_Group LIKE ?)";
    $like = "%$q%";
...
$pageSubtitle = 'Add, update and track your hospital\'s available vaccines and stock.';
include '../includes/hospital_header.php';
?>

<div class="card-panel">
  <div class="card-panel-head">
    <h3>My Vaccine Inventory <span class="id-chip"><?= count($vaccines) ?> total</span></h3>
    <div class="toolbar">
    ...
    <form method="POST" id="vaccineForm">
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" name="vaccine_name" id="vaccineName" pattern="^(?=.*[A-Za-z])[A-Za-z0-9\s\-]{2,100}$" title="Enter a valid vaccine name (must contain letters)" required>
        </div>
        <div class="form-group">
          <label class="form-label">Description</label>
          <textarea class="form-control" name="description" id="vaccineDescription" rows="3" style="resize: none;"></textarea>
        </div>
