...
foreach ($trend as $t) {
  if (isset($trendMap[$t['ym']])) $trendMap[$t['ym']] = (int)$t['c'];
}
...
<div style="display:grid;grid-template-columns:1.4fr 1fr;gap:20px;align-items:start;margin-top:20px;" class="dash-grid">
      ...
      <table class="data-table">
        ...
        <tbody>
          <?php if (!$recentBookings): ?>
            <tr>
              <td colspan="5">
                <div class="empty-state" style="padding:24px;"><i class="fa-solid fa-inbox"></i>
  ...
  <div class="card-panel">
    ...
    <div class="card-panel-body" style="padding-top:10px;">
      <?php if (!$vaccineStock): ?>
        <div class="empty-state" style="padding:20px;"><i class="fa-solid fa-vial"></i>
          <h4>No vaccines added</h4>
        </div>
      <?php endif; ?>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    // 1. Check if Chart object loaded properly
    ...
    if (trendCtx) {
      new Chart(trendCtx, {
        type: 'line',
        data: {
          labels: <?= json_encode(array_values($trendLabels)) ?>,
          datasets: [{
            label: 'Vaccinations completed',
            data: <?= json_encode(array_values($trendData), JSON_NUMERIC_CHECK) ?>,
            borderColor: '#2563EB',
    ...
    // 3. Status Breakdown Chart Setup
    const statusCtx = document.getElementById('statusChart');
    if (statusCtx) {
      new Chart(statusCtx, {
        type: 'doughnut',
        data: {
          labels: <?= json_encode(array_values($statusLabels)) ?>,
          datasets: [{
            data: <?= json_encode(array_values($statusData), JSON_NUMERIC_CHECK) ?>,
