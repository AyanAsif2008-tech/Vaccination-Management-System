...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_status') {
        ...
        // If the admin just approved this hospital, email them the good news.
        if ($status === 'Approved') {
            $stmt2 = $pdo->prepare("SELECT Email, Hospital_Name FROM hospital WHERE Hospital_ID = ?");
            $stmt2->execute([$id]);
            $hospitalRow = $stmt2->fetch();

            if ($hospitalRow) {
                send_hospital_approval_email($hospitalRow['Email'], $hospitalRow['Hospital_Name']);
            }
        }
      ...
      <div class="modal-body modal-lg">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="hospital_name" id="hospitalName" pattern="^(?=.*[A-Za-z])[A-Za-z0-9\s.,'\-&]{2,150}$" title="Enter a valid hospital name (must contain letters)" required>
          </div>
          <div class="form-group">
            <label class="form-label">Location / City</label>
            <input type="text" class="form-control" name="location" id="hospitalLocation" pattern="[A-Za-z\s,\-]{2,100}" title="City/region name: letters, spaces, commas and hyphens only">
          </div>
        </div>

        <div class="form-group">
          <label class="form-label">Address</label>
          <textarea class="form-control" name="address" id="hospitalAddress" rows="2" required style="resize: none;"></textarea>
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="form-group">
            <label class="form-label">Phone</label>
            <input type="tel" class="form-control" name="phone" id="hospitalPhone" pattern="(0[0-9]{10}|\+92[0-9]{10}|\+91[0-9]{10})" title="Enter an 11-digit number starting with 0 (e.g. 03001234567), or +92/+91 followed by 10 digits">
          </div>
          <div class="form-group">
            <label class="form-label">Email</label>
            <input type="email" class="form-control" name="email" id="hospitalEmail" pattern="[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}" title="Enter a valid email address" required>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="username" id="hospitalUsername" pattern="^[A-Za-z0-9_]{3,30}$" title="3-30 characters: letters, numbers, and underscores only" required>
          </div>
          <div class="form-group">
