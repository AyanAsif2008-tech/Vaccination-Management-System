...
/**
 * Checks whether a phone number is already used by ANY existing parent
 * or hospital account, so the same number can't be registered twice —
 * whether the new account being created is a parent or a hospital.
 */
function is_phone_taken($pdo, $phone) {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM parent WHERE Phone = ?");
    $stmt->execute([$phone]);
    if ($stmt->fetchColumn() > 0) {
        return true;
    }

    $stmt = $pdo->prepare("SELECT COUNT(*) FROM hospital WHERE Phone = ?");
    $stmt->execute([$phone]);
