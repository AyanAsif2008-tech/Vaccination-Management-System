...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($old['name'] === '' || $old['email'] === '' || $old['username'] === '' || $password === '') {
    ...
    } elseif ($old['phone'] !== '' && is_phone_taken($pdo, $old['phone'])) {
        ...
        try {
            ...
            // Registration succeeds but does NOT log the parent in automatically —
            // they must sign in via the central login page.
...
<title>Create Account · Vaccination Management System</title>
...
<div class="login-shell">
  ...
  <div class="login-visual">
    <div class="login-card-brand">
      ...
      <div class="brand-word">Vacci Track<small>Parent Registration</small></div>
    </div>
    <div>
      <h1 style="font-weight: bold;">Keep every dose, every date, in one place.</h1>
    ...
    <div class="login-form-box">
      ...
      <form method="POST" action="register.php">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" name="name" value="<?= e($old['name']) ?>" pattern="^(?=.*[A-Za-z])[A-Za-z\s'\-]{2,100}$" title="Enter a valid name (letters only, at least 2 characters)" required>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="form-group">
            <label class="form-label">Email</label>
            <input type="email" class="form-control" name="email" value="<?= e($old['email']) ?>" pattern="[A-Za-z0-9]([A-Za-z0-9._%+\-]*[A-Za-z0-9])?@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}" title="Enter a valid email address (it can't start, end, or consist only of dots)" required>
          </div>
          <div class="form-group">
            <label class="form-label">Phone</label>
            <input type="tel" class="form-control" name="phone" value="<?= e($old['phone']) ?>" pattern="(0[0-9]{10}|\+92[0-9]{10}|\+91[0-9]{10})" title="Enter an 11-digit number starting with 0 (e.g. 03001234567), or +92/+91 followed by 10 digits">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Username</label>
          <input type="text" class="form-control" name="username" value="<?= e($old['username']) ?>" pattern="^[A-Za-z0-9_]{3,30}$" title="3-30 characters: letters, numbers, and underscores only" required>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="form-group">
            <label class="form-label">Password</label>
            <input type="password" class="form-control" name="password" pattern="(?=.*[A-Za-z])(?=.*\d).{6,}" title="Password must be at least 6 characters and contain at least one letter and one number" required minlength="6">
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" name="address" rows="2" style="resize: none;"><?= e($old['address']) ?></textarea>
        </div>
