...
// Which hospital carries which vaccine, and its current stock status 
// shown as a reference table so parents can check availability before booking.
$vaccineAvailability = $pdo->query("
    SELECT h.Hospital_Name, v.Vaccine_Name, v.Age_Group, v.Stock_Status
    FROM vaccine v
    JOIN hospital h ON h.Hospital_ID = v.Hospital_ID
...
<div class="card-panel" style="max-width:1200px;margin-bottom:20px;">
  <div class="card-panel-head"><h3>Vaccine Availability by Hospital</h3></div>
  <div class="table-wrap">
    <table class="data-table">
      <thead>
        <tr><th>Hospital</th><th>Vaccine</th><th>Age Group</th><th>Status</th></tr>
      </thead>
      <tbody>
      <?php if (!$vaccineAvailability): ?>
        <tr><td colspan="4"><div class="empty-state"><i class="fa-solid fa-vial-circle-check"></i><h4>No vaccine data available yet</h4></div></td></tr>
      <?php endif; ?>
      <?php foreach ($vaccineAvailability as $va): ?>
        <tr>
          <td class="cell-primary"><?= e($va['Hospital_Name']) ?></td>
          <td><?= e($va['Vaccine_Name']) ?></td>
          <td><?= e($va['Age_Group']) ?: '—' ?></td>
          <td><?= status_badge($va['Stock_Status']) ?></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
...
<?php else: ?>
<div class="card-panel" style="max-width:1200px;">
